defmodule AshMix.Task.GetTest do
  @moduledoc false
  ...
  describe "when the record exists" do
    ...
    test "it can be retrieved by it's public key", %{post: post} do
      output =
        capture_io(fn ->
          Mix.Task.rerun("ash_mix.example.get_post", [to_string(post.id)])
        end)
    ...
    test "it can use a provided tenant", %{post: post} do
      output =
        capture_io(fn ->
          Mix.Task.rerun("ash_mix.example.get_post", ["--tenant", post.tenant, to_string(post.id)])
        end)
    ...
    test "when the provided tenant is invalid, it fails", %{post: post} do
      output =
        capture_io(:stderr, fn ->
          Mix.Task.rerun("ash_mix.example.get_post", [
            "--tenant",
            "Marty McFly",
            to_string(post.id)
          ])
    ...
    test "it can format the output as JSON", %{post: post} do
      output =
        capture_io(fn ->
          Mix.Task.rerun("ash_mix.example.get_post", ["--format", "json", to_string(post.id)])
        end)
    ...
    test "when the post doesn't exist, it fails" do
      output =
        capture_io(:stderr, fn ->
          Mix.Task.rerun("ash_mix.example.get_post", [to_string(Ash.UUID.generate())])
        end)
